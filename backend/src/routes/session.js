...
router.post('/:id/map-moments', async (req, res) => {
  const { data: session, error } = await supabase
    .from('session_journals')
    .select('id, title, transcript, voice_memo_text, duration_ms')
    .eq('id', req.params.id)
  ...
  const transcript = session.transcript || session.voice_memo_text || ''
  ...
  setImmediate(async () => {
    ...
    try {
      ...
      console.log('[map-moments] job=' + jobId + ' — ' + lines.length + ' lines in ' + chunks.length + ' chunks')
      ...
      allMoments.sort((a, b) => {
        const toSecs = t => { const m = t && t.match(/(\d+):(\d+)/); return m ? parseInt(m[1])*60+parseInt(m[2]) : 0 }
        return toSecs(a.timecode) - toSecs(b.timecode)
      })

      await supabase
        .from('session_journals')
        .update({ key_moments: allMoments.map(m => m.timecode + ' [' + m.type + '] ' + m.summary), updated_at: new Date().toISOString() })
        .eq('id', req.params.id)
        .eq('user_id', req.user.id)
